...
int n,q;
vector<ll> arr;
vector<vector<Info>> arr2(21,vector<Info>());
void solve(){
    cin >> n;
    arr.assign(n,0);
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int j = 0; j < 21; j++){
            if (arr[i]&((ll)1<<j)){
                arr2[j].push_back(Info(1));
            }else{
                arr2[j].push_back(Info(0));
            }
        }
    }
    // for(auto i:arr2){
    ...
    //     cout << nl;
    // }
    vector<LazySegmentTree<Info,Tag>> segs;
    for(int i = 0; i < 21; i++){
        segs.push_back(LazySegmentTree<Info,Tag>(arr2[i]));
    }
    cin >> q;
    ll t,l,r,x;
    while(q--){
        ...
        if(t == 1){
            ...
            ll ans = 0;
            for(int i = 0; i < 21; i++){
                ans += ((ll)1<<i)*segs[i].Query(l,r).cur;
            }
            cout << ans << nl;
        }else{
            cin >> l >> r >> x;
