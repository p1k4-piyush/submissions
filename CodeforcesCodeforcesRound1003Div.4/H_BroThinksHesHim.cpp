...
void evermore()
{
    ...
    n = s.size();
    Mint ans = 0;
    FenwickTree<Mint> pre_zero(n), pre_one(n), suf_zero(n), suf_one(n);

    for (int i = 0; i < n; i++) {
        if (s[i] == '0') {
            ans += pre_one.Query(i) * power2[n - i - 1];
            pre_zero.Modify(i, power2[i]);
            suf_zero.Modify(i, power2[n - i - 1]);
        } else {
            ans += pre_zero.Query(i) * power2[n - i - 1];
            pre_one.Modify(i, power2[i]);
            suf_one.Modify(i, power2[n - i - 1]);
        }
    }
    ans += power2[n];
    ans--;
    ...
    for (int tt = 0; tt < q; tt++) {
        ...
        if (s[t] == '0') {
            s[t] = '1';
            ans -= pre_one.Query(t) * power2[n - t - 1];
            dbg(ans);
            ans -= (suf_one.Query(n) - suf_one.Query(t)) * power2[t];
            dbg(ans);
            pre_zero.Modify(t, -power2[t]);
            suf_zero.Modify(t, -power2[n - t - 1]);
            ans += pre_zero.Query(t) * power2[n - t - 1];
            dbg(ans);
            ans += (suf_zero.Query(n) - suf_zero.Query(t)) * power2[t];
            dbg(ans);
            pre_one.Modify(t, power2[t]);
            suf_one.Modify(t, power2[n - t - 1]);
        } else {
            s[t] = '0';
            ans -= pre_zero.Query(t) * power2[n - t - 1];
            dbg(ans);
