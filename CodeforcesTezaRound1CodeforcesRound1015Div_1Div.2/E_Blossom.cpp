...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(c == 1){
            for(int j = 0; j < k; j++){
                int cur = v[j];
                tt += Mint(min(l,(ll)cur))*Mint(n-max(r,(ll)cur)+1);
            }
        }else{
            for(int j = 0; j < k; j++){
                for(int j2 = j+1; j2 < k; j2++){
                    int t1 = j2-j-1;
                    Mint m = C(t1,c-2);
                    if (m == Mint(0)) continue;
                    Mint l1 = min(l,(ll)v[j]);
                    Mint r1 = max(r,(ll)v[j2]);
                    tt += m*(l1*Mint(n-r1+1));
                }
            }
        }
        dp[i+1] = tt*fact[k-c]*fact[c];
    }
    
    dbg(dp);
    Mint ans = 0;
    for(int i = 0; i < n; i++){
        ans += Mint(i)*(dp[i]-dp[i+1]);
    }
    ans += dp[n]*n;
