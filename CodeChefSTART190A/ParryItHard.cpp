...
void evermore()
{
    ...
    SimpleSegmentTree<Info> seg(a);
    vector<int> mx(n, 0);
    int cur = x;
    for (int i = 0; i < n; i++) {
        if ((cur >= b[i]) && ((cur - 1) >= seg.Query(i + 1, n).cur)) {
            mx[i] = 1;
            cur--;
        }
    }
    partial_sum(mx.begin(), mx.end(), mx.begin());
    int M = mx[n - 1];
    vector<int> mx2(n + 1, 0);
    vector<bool> ans(n, 0);
    vector<int> tt;
    for (int i = n - 1; i >= 0; i--) {
        if ((x - M + mx2[i + 1] + 1 >= b[i]) && (mx2[i + 1] < M)) {
            ans[i] = 1;
            mx2[i]++;
            tt.push_back(i);
        }
    ...
    for (int i = 0; i < n; i++) {
        ...
        int rem = mx2[i];
        if ((upper_bound(tt.begin(), tt.end(), i) != tt.end()) && (b[i] <= x + rem - M) && (x + rem - M - 1) >= seg.Query(i + 1, *upper_bound(tt.begin(), tt.end(), i) + 1).cur) {
            cout << 1;
        } else {
            cout << 0;
        }
...
// time-limit: 1000
// problem-url: https://www.codechef.com/START190A/problems/MXPARR
