...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        if(arr[i]==0){
            ...
            if(arr[i]>=2)   dp[i][arr[i]-1] += dp[i-1][arr[i]-2];
            dp[i][arr[i]-1] += dp[i-1][arr[i]-1];
            if(arr[i]<=m-1)   dp[i][arr[i]-1] += dp[i-1][arr[i]];
        } 
    }

    dbg(dp);
    
    cout << accumulate(dp[n-1].begin(),dp[n-1].end(),Mint(0)) << nl;
    return;
}
