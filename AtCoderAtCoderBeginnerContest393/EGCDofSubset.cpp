...
vector<T> BuildDivisorsFromFactors(const vector<pair<T, int>>& factors) {
  ...
  sort(divisors.begin(), divisors.end(),greater<T>());
  return divisors;
}
...
void solve(){
    ...
    vector<vector<int>> divisors_list(mx+1);
    for(int i = 1; i <= mx; i++){
        for(int j = i; j <= mx; j += i){
    ...
    dbg(divisors_list);
    for(auto i:arr){
        // vector<int> divisors = factorizer::BuildDivisorsFromFactors(factorizer::Factorize(i));
        for(int j = divisors_list[i].size() - 1; j >= 0; j--){
            if (cnt[divisors_list[i][j]]>=k){
                cout << divisors_list[i][j] << nl; break;
            }
        }
