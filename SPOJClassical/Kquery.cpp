...
void solve(){
    ...
    sort(queries.begin(),queries.end(),greater<pair<pair<int,pair<int,int>>,int>>());

    int p = 0;
    for(auto i:queries){
        while(p < n && points[p].first > i.first.first){
            seg.Set(points[p].second,1);
            p++;
        }
        ans[i.second] = seg.Query(i.first.second.first,i.first.second.second).cur;
    }
