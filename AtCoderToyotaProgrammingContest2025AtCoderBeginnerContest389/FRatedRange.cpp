...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        ...
        int t = seg.MaxRight(0,[&](Info tt){return tt.cur<l;});
        int t2 = seg.MaxRight(0,[&](Info tt){return tt.cur<r+1;});

        // cout << t << " " << t2 << nl;
        seg.Modify(t,t2,Tag(1));
    }
    cin >> q;
    int t;
    while(q--){
        cin >> t;
        cout << seg.Query(t,t+1).cur << nl;
    }
