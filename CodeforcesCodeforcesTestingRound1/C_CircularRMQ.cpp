...
void evermore(){
    ...
    string s;
    getline(cin,s);
    LazySegmentTree<Info, Tag> lazy(arr);
    while(m--){
        string s;
        getline(cin,s);
        dbg(s);
        ...
        if(v.size() == 3){
            if (v[0] > v[1]){
                lazy.Modify(v[0],n,Tag(v[2]));
                lazy.Modify(0,v[1]+1,Tag(v[2]));
            }else{
                lazy.Modify(v[0],v[1]+1,Tag(v[2]));
            }
        }else{
            if(v[0] > v[1]){
                Info t = lazy.Query(v[0],n);
                Info tt = lazy.Query(0,v[1]+1);
                cout << min(t.cur,tt.cur) << nl;
            }else{
                Info t = lazy.Query(v[0],v[1]+1);
                cout << t.cur << nl;
            }
        }
    }
