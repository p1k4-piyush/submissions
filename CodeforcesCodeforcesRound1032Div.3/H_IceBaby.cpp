...
void evermore()
{
    ...
    LazySegmentTree<Info, Tag> lazy(tt);
    for (int i = 0; i < n; i++) {
        int ll = mp[l[i]];
        int rr = mp[r[i]];
        int t = lazy.Query(0, ll).cur;
        int t2 = lazy.Get(ll).cur;
        if (t > t2) {
            lazy.Set(ll, Info(t));
        }

        lazy.Modify(ll, rr + 1, Tag(1));
        cout << lazy.Query(0, tt).cur << " ";
    }
...
// time-limit: 2000
// problem-url: https://codeforces.com/contest/2121/problem/H
