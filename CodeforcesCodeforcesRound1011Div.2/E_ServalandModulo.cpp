...
void solve(){
    ...
    ll s = accumulate(a.begin(),a.end(),(ll)0) - accumulate(b.begin(),b.end(),(ll)0);
    if (s == ll(0)){
        if(checker(ll(31415926))){
            cout << "31415926" << nl;
            return;
        }else{
            cout << "-1" << nl;
    ...
    vector<ll> v = factorizer::BuildDivisorsFromFactors(factorizer::Factorize(s));
    dbg(v);
    for (auto i:v){
        if (checker(i)){
            cout << i << nl; return;
...
// time-limit: 2000
// problem-url: https://codeforces.com/contest/2085/problem/E
