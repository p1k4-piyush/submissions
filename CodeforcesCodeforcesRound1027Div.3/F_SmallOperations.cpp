...
void evermore()
{
    ...
    vector<int> v = factorizer::BuildDivisorsFromFactors(factorizer::Factorize(x / g));
    vector<int> dp(v.size(), 1e9);
    dp[0] = 0;
    for (int i = 0; i < v.size(); i++) {
        for (int j = i - 1; j >= 0; j--) {
            if ((v[i] / v[j] <= k) && (v[i] % v[j] == 0)) {
    ...
    v = factorizer::BuildDivisorsFromFactors(factorizer::Factorize(y / g));
    dp.assign(v.size(), 1e9);
    dp[0] = 0;
    for (int i = 0; i < v.size(); i++) {
        for (int j = i - 1; j >= 0; j--) {
            if ((v[i] / v[j] <= k) && (v[i] % v[j] == 0)) {
