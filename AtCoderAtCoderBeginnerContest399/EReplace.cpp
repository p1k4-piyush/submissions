...
template <typename T>
vector<vector<int>> find_cycles(const graph<T>& g, int bound_cnt = 1 << 30,
    int bound_size = 1 << 30)
{
    ...
    // digraph: finds at least one cycle in every connected component (if not
    // broken) undigraph: finds cycle basis
}
